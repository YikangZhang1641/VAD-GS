...
from lib.utils.waymo_utils import my_vis, create_custom_camera_visualization
...
class GaussianModel(nn.Module):
    ...
    def make_ply(self):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation), axis=1)#, semantic
    ...
    def weighted_random_sampling(self, world_coords, colors, normals, mask, acc, target_count=5000):
        H, W = mask.shape

        # Flatten
        world_coords_flat = world_coords.view(-1, 3)
        ...
        # Apply mask
        valid_coords = world_coords_flat[mask_flat]
        valid_colors = colors_flat[mask_flat]
        valid_acc = acc_flat[mask_flat]
        valid_normals = normals_flat[mask_flat]

        N = valid_coords.shape[0]
        if N == 0:
            return torch.empty(0, 3), torch.empty(0, 3)

        # Compute inverse weight: low acc => high weight
        # weights = 1.0 - valid_acc  # acc越小权重越大

        weights = torch.where(valid_acc < 0.4, 1.0, torch.where(valid_acc < 0.7, 0.3, torch.where(valid_acc < 0.9, 0.1, 0.05)))
        # print("new")
        weights = weights.clamp(min=1e-6)  # 避免为0
        weights = weights / weights.sum()  # 归一化

        # Weighted sampling
        if N > target_count:
            sample_indices = torch.multinomial(weights, target_count, replacement=False)
            valid_coords = valid_coords[sample_indices]
            valid_colors = valid_colors[sample_indices]
            valid_normals = valid_normals[sample_indices]

        return valid_coords, valid_colors, valid_normals


    def densify_from_depth_propagation(self, K, cam2target, propagated_depth, propagated_normal, filter_mask, acc, gt_image, init_opacity=0.5, target_count=2000):
        ...
        propagated_normal_cam = -propagated_normal[[0,2,1], :, :].permute(1,2,0).view(-1, 3)
        world_normal = (cam2target[:3, :3] @ propagated_normal_cam.T).T
        world_normal = world_normal.view(height, width, 3)

        gt_image_permuted = gt_image.permute(1, 2, 0)

################# visualize if correct #######################
        # import open3d as o3d
        # world_origin = o3d.geometry.PointCloud()
        # world_origin.points = o3d.utility.Vector3dVector(self.get_xyz.cpu().detach().numpy())
        # # o3d.visualization.draw_geometries([world_origin])

        # # points_xyz = world_coordinates_3D[filter_mask].detach().cpu().numpy()
        # # points_clr = gt_image_permuted[filter_mask].detach().cpu().numpy()
        # # points_n = world_normal[filter_mask].cpu().detach().numpy()
        ...
        # cam_meshs = create_custom_camera_visualization(
        #     view_width_px=width,
        #     view_height_px=height,
        #     intrinsic=K[:3, :3].cpu().detach().numpy(),
        #     extrinsic=np.eye(4),
        #     scale=1,
        #     line_radius=0.03,
        #     color=[1,0,0]
        ...
        # # o3d.visualization.draw_geometries([point_cloud, world_origin, line_set, FOR1])
        # o3d.visualization.draw_geometries([point_cloud, line_set] + cam_meshs)
##################################################
        # #mask the points below the confidence threshold
        # #downsample the pixels; 1/4
        # if filter_mask[::8, ::8].sum() > 100:
        #     step_size = 8
        # elif filter_mask[::4, ::4].sum() > 100:
        #     step_size = 4
        # elif filter_mask[::2, ::2].sum() > 100:
        #     step_size = 2
        # else:
        #     step_size = 1

        # world_coordinates_3D_downsampled = world_coordinates_3D[::step_size, ::step_size]
        # filter_mask_downsampled = filter_mask[::step_size, ::step_size]
        # gt_image_downsampled = gt_image_permuted[::step_size, ::step_size]
        ...
        world_coordinates_3D_downsampled, color_downsampled, world_normal_downsampled = self.weighted_random_sampling(
            world_coords=world_coordinates_3D,
        ...
        # import open3d as o3d
        # point_cloud = o3d.geometry.PointCloud()
        # point_cloud.points = o3d.utility.Vector3dVector(world_coordinates_3D_downsampled.detach().cpu().numpy())
        ...
        # point_cloud_origin.colors = o3d.utility.Vector3dVector(np.ones_like(original_point_cloud.detach().cpu().numpy()) * 0.7)
        ...
        axis = torch.vstack([-world_normal_downsampled[:,1], world_normal_downsampled[:,0], torch.zeros_like(world_normal_downsampled[:,0])]).T
        # axis_norm = np.linalg.norm(axis, axis=1)
        # mask = axis_norm > 1e-6
        # axis[mask] /= axis_norm[mask].reshape(-1,1)
        # axis[~mask][:, 0] = 0
        # axis[~mask][:, 1] = 0
        # axis[~mask][:, 2] = 1
        # half_theta = np.arccos(world_normal_downsampled[:,2]) / 2
        # q_w = np.cos(half_theta).reshape(-1,1)
        # q_xyz = axis * np.sin(half_theta).reshape(-1,1)
        # rots = torch.from_numpy(np.concatenate([q_w, q_xyz], axis=1)).float().cuda()
        ...
        axis[~mask][:, 1] = 0
        axis[~mask][:, 2] = 1
        half_theta = torch.arccos(world_normal_downsampled[:,2:3]) / 2
    ...
    def visualize_gaussian_centers(self):
        import open3d as o3d
        pcd = o3d.geometry.PointCloud()
        xyz = self.get_xyz.cpu().detach().numpy()
        color = SH2RGB(self._features_dc).cpu().detach().numpy()[:,0,:]

        pcd.points = o3d.utility.Vector3dVector(xyz)
        pcd.colors = o3d.utility.Vector3dVector(color)
        voxels = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd, voxel_size=0.15)

        FOR1 = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1, origin=[0, 0, 0])
        o3d.visualization.draw_geometries([voxels, FOR1])
