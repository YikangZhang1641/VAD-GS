...
class StreetGaussianRenderer():
    ...
    def render_kernel(
        ...
        if render_type == "hard_depth":
            opacity = torch.ones(opacity.shape[0], 1, device=opacity.device) # * 0.95
        ...
        if cfg.render.render_normal and render_type != "hard_depth":
            ...
            normals = normals * ((((view_dir * normals).sum(dim=-1) < 0) * 1 - 0.5) * 2)[...,None]

            R_w2c = torch.tensor(viewpoint_camera.R.T).cuda().to(torch.float32)
            normals = (R_w2c @ normals.transpose(0, 1)).transpose(0, 1)
