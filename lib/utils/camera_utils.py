...
def loadguidance(guidance, resolution): # 这里不载入cuda是因为全部装进去会爆显存
    new_guidance = dict()
    for k, v in guidance.items():
        if k == 'mask':
            ...
            new_guidance['dynamic_mask'] = NumpytoTorch(v, resolution, resize_mode=Image.NEAREST).int()
        elif k=="mono_depth":
            new_guidance['mono_depth'] = PILtoTorch(v, resolution, resize_mode=Image.NEAREST).to(torch.float)
        elif k=="mono_normal":
            new_guidance['mono_normal'] = NumpytoTorch(v, resolution, resize_mode=Image.NEAREST).to(torch.float)
        elif k=="seg_bkgd":
            new_guidance["seg_bkgd"] = NumpytoTorch(v, resolution, resize_mode=Image.NEAREST).int()
