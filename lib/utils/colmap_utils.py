...
def read_points3D_binary(path_to_model_file):
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for p_id in range(num_points):
            ...
            track_elems = []  # 存储观测该点的视角
            for _ in range(track_length):
                image_id = struct.unpack("<I", fid.read(4))[0]  # 图像 ID
                # fid.read(4)  # 忽略 feature_id
                point_id = struct.unpack("<I", fid.read(4))[0]  # POINT2D_IDX
                track_elems.append([image_id, point_id])

            xyzs[p_id] = xyz
            rgbs[p_id] = rgb
            errors[p_id] = error
            tracks[p_id] = track_elems
