...
class BaseVisualizer():
    def __init__(self, save_dir):
        ...
        self.depth_visualize_func = lambda x: visualize_depth_numpy(x, cmap=cv2.COLORMAP_INFERNO)[0][..., [2, 1, 0]]
    ...
    def visualize_depth(self, result, camera: Camera):
        ...
        if self.save_image:
            imageio.imwrite(os.path.join(self.result_dir, f'{name}_depth.png'), self.depth_visualize_func(depth.max() - depth))
    ...
    def visualize_normal(self, result, camera: Camera):
        ...
        # normal = normal.detach().permute(1, 2, 0).detach().cpu().numpy() # [H, W, 1]
        if self.save_image:
            torchvision.utils.save_image(normal, os.path.join(self.result_dir, f'{name}_normal.png'))
        
        if self.save_video:
            self.depths.append(normal)
