...
{
    float X[3];
    Get3DPoint(camera, p, depth, X);  // 相机坐标系（右，下，前）
...
__device__ float4 GenerateRandomNormal(const Camera camera, const int2 p, curandState *rand_state, const float depth, const float3 normal_guess)
{
    float4 normal;
    normal.w = 0;  
    if ((normal_guess.x > 0.99 && normal_guess.y > 0.99 && normal_guess.z > 0.99) || 
        (normal_guess.x < -0.99 && normal_guess.y < -0.99 && normal_guess.z < -0.99) || 
        (-0.01 < normal_guess.x && normal_guess.x < 0.01 &&  -0.01 < normal_guess.y && normal_guess.y < 0.01 && -0.01 < normal_guess.z && normal_guess.z < 0.01)
    ) { // ramdomize or use normal guess
        float q1 = 1.0f;
        float q2 = 1.0f;
        float s = 2.0f;
        while (s >= 1.0f) {
        ...
        normal.x = normal_guess.x;
        normal.y = normal_guess.y;
        normal.z = normal_guess.z;
    }
...
__device__ float4 GenerateRandomPlaneHypothesis(const Camera camera, const int2 p, curandState *rand_state, const float depth_min, const float depth_max, const float init_depth, const float init_anchor, const float3 normal_guess)
{
    float depth = init_depth; // zyk: delete later
    // if (init_anchor > 1.14) {
    //     depth = init_anchor;
    // } else 
    if (depth <= 0.1){
    ...
    float4 plane_hypothesis = GenerateRandomNormal(camera, p, rand_state, depth, normal_guess);
    plane_hypothesis.w = GetDistance2Origin(camera, p, depth, plane_hypothesis);
    return plane_hypothesis;
}

__device__ float4 GetDistance2OriginWithScaleShifts(const Camera camera, const int2 p, const float4 plane_hypothesis, const float scale_shift)
{
    float current_depth = plane_hypothesis.w * scale_shift;
    float4 anchored_plane_hypothesis;
    anchored_plane_hypothesis.x = plane_hypothesis.x;
    anchored_plane_hypothesis.y = plane_hypothesis.y;
    anchored_plane_hypothesis.z = plane_hypothesis.z;
    anchored_plane_hypothesis.w = GetDistance2Origin(camera, p, current_depth, plane_hypothesis);
    return anchored_plane_hypothesis;
}
...
__device__ float ComputeBilateralNCC(const cudaTextureObject_t ref_image, const Camera ref_camera, const unsigned char* ref_mask, const cudaTextureObject_t src_image, const Camera src_camera, const unsigned char* src_mask, const int2 p, const float4 plane_hypothesis, const PatchMatchParams params)
{
    ...
    const int p_center = p.y * src_camera.width + p.x;
    const int pt_center = pt.y * src_camera.width + pt.x;
    if (ref_mask[p_center] < 128 || src_mask[pt_center] < 128) { // zyk:  p点在ref和src落入mask则直接认为匹配错误
        return cost_max;
    }
    ...
    float cost = 0.0f;
    {
        ...
        for (int i = -radius; i < radius + 1; i += params.radius_increment) {
            ...
            for (int j = -radius; j < radius + 1; j += params.radius_increment) {
                ...
                float weight = ComputeBilateralWeight(i, j, ref_pix, ref_center_pix, params.sigma_spatial, params.sigma_color); // 像素相似度(sptial+color)
        ...
        if (var_ref < kMinVar || var_src < kMinVar) {// ref patch和src patch需要方差足够大（不能过于均匀，patch特征不明显）
            return cost = cost_max;
        } else {
            const float covar_src_ref = sum_ref_src - sum_ref * sum_src;
            const float var_ref_src = sqrt(var_ref * var_src);
            return cost = max(0.0f, min(cost_max, 1.0f - covar_src_ref / var_ref_src)); // rs协方差-r*s标准差 = 互信息NCC, 正相关时=1，无关时=0，负相关时<0
        }
    }
}

__device__ float ComputeMultiViewInitialCostandSelectedViews(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, unsigned int *selected_views, const PatchMatchParams params, const unsigned char** d_mask_array)
{
    float cost_max = 2.0f;
    ...
    for (int i = 1; i < params.num_images; ++i) {
        float c = ComputeBilateralNCC(images[0], cameras[0], d_mask_array[0], images[i], cameras[i], d_mask_array[i], p, plane_hypothesis, params); // 计算 p点 在images[0]的patch对其他images的平面假设. 高相关性趋近0，低相关性趋近1，无关为default 2
        cost_vector[i - 1] = c;
    ...
    sort_small(cost_vector, cost_count); // 小的在前面
    *selected_views = 0; 

    int top_k = min(num_valid_views, params.top_k); // p点在每个src图中有效的个数
    if (top_k > 0) {
        float cost = 0.0f;
        ...
        for (int i = 0; i < params.num_images - 1; ++i) {
            if (cost_vector_copy[i] <= cost_threshold) { // cost_vector经过sort后id为乱序，所以保留一份cost_vector_copy用于view_id记录
                setBit(*selected_views, i); // selected_views 为32位int，故最大视角数为32。认为观测正确时置1
            }
        }
        return cost / top_k;  // 该hypothesis对于p点在各有效src视角的NCC指标均值
    } else {
        return cost_max;
    }
}

__device__ void ComputeMultiViewCostVector(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, float *cost_vector, const PatchMatchParams params, const unsigned char** d_mask_array) 
{
    for (int i = 1; i < params.num_images; ++i) {
        cost_vector[i - 1] = ComputeBilateralNCC(images[0], cameras[0], d_mask_array[0], images[i], cameras[i], d_mask_array[i], p, plane_hypothesis, params); // 计算 p点 在images[0]的patch对其他images的平面假设. 高相关性趋近0，低相关性趋近1，无关为default 2
    }
}
...
__global__ void RandomInitialization(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses, 
                                     ...
                                     const PatchMatchParams params, float *depths, float *anchors, float3* normals, unsigned char** d_mask_array) // 注意，此处是global
{
    ...
    if (!params.geom_consistency) { // geom_consistency 为 False，进这里
        plane_hypotheses[center] = GenerateRandomPlaneHypothesis(cameras[0], p, &rand_states[center], params.depth_min, params.depth_max, depths[center], anchors[center], normals[center]); //zyk: sparse anchors // 采样normal；若depth<=0，采样；计算distance
    }
    else {
        float4 plane_hypothesis = plane_hypotheses[center]; // ses 大约是世界坐标系的normal朝向 + depth（而非distance）
        plane_hypothesis = TransformNormal2RefCam(cameras[0], plane_hypothesis); 
        float depth = plane_hypothesis.w;
        plane_hypothesis.w = GetDistance2Origin(cameras[0], p, depth, plane_hypothesis); // sis 计算ref系下normal和distance
        plane_hypotheses[center] = plane_hypothesis;
    }
    costs[center] = ComputeMultiViewInitialCostandSelectedViews(texture_objects[0].images, cameras, p, plane_hypotheses[center], &selected_views[center], params, d_mask_array); // 该hypothesis对于p点在各有效src视角的NCC指标均值
}


__global__ void RefineHypothesisWithScaleShifts(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses, float *anchors)
{
    const int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    int width = cameras[0].width;
    int height = cameras[0].height;
    ...
    // const int center = p.y * width + p.x; // not finished 需要一个text2D格式的anchor depth或shift

    // plane_hypotheses[center] = GetDistance2OriginWithScaleShifts(cameras[0], p, plane_hypotheses[center], scale_shift) // depth -> scale shifted depth -> distance
}


__device__ void PlaneHypothesisRefinement(const cudaTextureObject_t *images, const cudaTextureObject_t *depth_images, const Camera *cameras, const unsigned char** d_mask_array, float4 *plane_hypothesis, float *depth, float *cost, curandState *rand_state, const float *view_weights, const float weight_norm, const int2 p, const PatchMatchParams params, float* anchor)
{
    // 随机几个猜测，在当前值附近扰动几个猜测，看看会不会更好
    float perturbation = 0.02f; 

    float depth_rand = curand_uniform(rand_state) * (params.depth_max - params.depth_min) + params.depth_min;
    float3 normal_guess = make_float3(1,1,1);
    float4 plane_hypothesis_rand = GenerateRandomNormal(cameras[0], p, rand_state, *depth, normal_guess);
    ...
    // 对depth和normal进行2%范围内的扰动

    const int num_planes = 5; // 随d正n、正d随n、随d随n、正d扰n、扰d正n
    float depths[num_planes] = {depth_rand, *depth, depth_rand, *depth, depth_perturbed};
    float anchor_value = *anchor;
    if (anchor_value > 1.0) { // zyk: given non-zero anchor value, fix all depths.
        for (int i = 0; i < num_planes; ++i) {
            depths[i] = anchor_value;
        }
    }
    ...
    for (int i = 0; i < num_planes; ++i) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, temp_plane_hypothesis, cost_vector, params, d_mask_array);
        ...
        if (depth_before >= params.depth_min && depth_before <= params.depth_max && temp_cost < *cost) {  // 如果refine的猜测更好，替换depth，hypo，cost
            *depth = depth_before;
...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images, const cudaTextureObject_t *depths, const Camera *cameras, 
                                        ...
                                        const int2 p, const PatchMatchParams params, const int iter, float *anchor_depth_cuda, const unsigned char** d_mask_array) 
{
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
    ...
    if (p.y > 2) {
        ...
        up_far = costMinPoint; // 找到其中cost最优的候选点 
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_far], cost_array[1], params, d_mask_array);  // 计算结果存在cost_array[1]，其32位分别代表观测视角
    }
    ...
    if (p.y < height - 3) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_far], cost_array[3], params, d_mask_array);
    }
    ...
    if (p.x > 2) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_far], cost_array[5], params, d_mask_array);
    }
    ...
    if (p.x < width - 3) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_far], cost_array[7], params, d_mask_array);
    }
    ...
    if (p.y > 0) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_near], cost_array[0], params, d_mask_array);
    }
    ...
    if (p.y < height - 1) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_near], cost_array[2], params, d_mask_array);
    }
    ...
    if (p.x > 0) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_near], cost_array[4], params, d_mask_array);
    }
    ...
    if (p.x < width - 1) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_near], cost_array[6], params, d_mask_array);
    }
    ...
    for (int i = 0; i < 4; ++i) { 
        if (flag[2 * i]) { // 对近处V字处理
            for (int j = 0; j < params.num_images - 1; ++j) {
                if (isSet(selected_views[neighbor_positions[i]], j) == 1) {  // 如果neighbor在hypothesis阶段认为观测正确
    ...
    float sampling_probs[32] = {0.0f};  // 对各个视角p点对应投影patch设计概率权重, p点假设的NCC越准，值越大
    ...
    TransformPDFToCDF(sampling_probs, params.num_images - 1); // 由pdf转cdf，归一化至0-1, 替换原sampling_probs
    ...
    } // sampling_probs为cdf，按rand_states采样，只访问落进的区间
    ...
    } // final_costs 计算从八个方向propagate来的加权NCC

    const int min_cost_idx = FindMinCostIndex(final_costs, 8);

    float cost_vector_now[32] = {2.0f};
    ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[center], cost_vector_now, params, d_mask_array); // 计算在当前几何假设下，center点在其他视角投影点的NCC相似度
    float cost_now = 0.0f;
    for (int i = 0; i < params.num_images - 1; ++i) {
        if (params.geom_consistency) {
    ...
    if (anchor_depth_cuda[center] > 1.0) { // zyk: depth fixed by anchor
        depth_now = anchor_depth_cuda[center];
    } else if (flag[min_cost_idx]) {  // 如果有propogation结果, 尝试传递猜测
        float depth_before = ComputeDepthfromPlaneHypothesis(cameras[0], plane_hypotheses[positions[min_cost_idx]], p);

        if (depth_before >= params.depth_min && depth_before <= params.depth_max && final_costs[min_cost_idx] < cost_now) { // 如果propogation提供hypothesis的NCC结果优于center，进行propogate, 替换hypo，cost和views
    ...
    PlaneHypothesisRefinement(images, depths, cameras, d_mask_array, &plane_hypotheses[center], &depth_now, &costs[center], &rand_states[center], view_weights, weight_norm, p, params, &anchor_depth_cuda[center]); // 随机加扰动, 看看是否会采样到更好的ND
}
...
__global__ void BlackPixelUpdate(cudaTextureObjects *texture_objects, cudaTextureObjects *texture_depths, 
                                 ...
                                 unsigned int *selected_views, const PatchMatchParams params, const int iter, float *anchor_depth_cuda, unsigned char** d_mask_array) 
{
    ...
    CheckerboardPropagation(texture_objects[0].images, texture_depths[0].images, cameras, plane_hypotheses, costs, rand_states, selected_views, p, params, iter, anchor_depth_cuda, d_mask_array); 
}
...
__device__ void CheckerboardFilter(const Camera *cameras, float4 *plane_hypotheses, float *costs, const int2 p)
{
    ...
    int median_index = index / 2; // 更新depth, 在propogate范围内的中位数
    if (index % 2 == 0) {
...
void Propagation::RunPatchMatch()
{
    ...
    // 随机生成一个normal的hypothesis，必要时采样depth, 并对p点在各src视角计算NCC指标均值  // 后续可以加入normal hypothesis  // 当前 propogate 过程中使用的是dist
    RandomInitialization<<<grid_size_randinit, block_size_randinit>>>(texture_objects_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, depths_cuda, anchor_depth_cuda, normals_guess_cuda, d_mask_array); 
    CUDA_SAFE_CALL(cudaDeviceSynchronize());


    for (int i = 0; i < max_iterations; ++i) {
        BlackPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, i, anchor_depth_cuda, d_mask_array); 
        CUDA_SAFE_CALL(cudaDeviceSynchronize());

        RedPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, i, anchor_depth_cuda, d_mask_array); 
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        // printf("iteration: %d\n", i);
    }

    GetDepthandNormal<<<grid_size_randinit, block_size_randinit>>>(cameras_cuda, plane_hypotheses_cuda, params); // 转回世界坐标系的N和D
    CUDA_SAFE_CALL(cudaDeviceSynchronize());

    BlackPixelFilter<<<grid_size_checkerboard, block_size_checkerboard>>>(cameras_cuda, plane_hypotheses_cuda, costs_cuda); // 更新depth, 在propogate范围内取中位数
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
