...
void saveFloatMatAsPNG(const cv::Mat& mat, const std::string& filename) {
    cv::Mat mat8U;
    cv::normalize(mat, mat8U, 0, 255, cv::NORM_MINMAX);  // 归一化到 0-255
    mat8U.convertTo(mat8U, CV_8U);  // 转换为 CV_8U
    cv::imwrite(filename, mat8U);
}

void saveFloat3ChannelMatAsPNG(const cv::Mat& mat, const std::string& filename) {
    cv::Mat mat8U;
    cv::normalize(mat, mat8U, 0, 255, cv::NORM_MINMAX);  // 归一化
    mat8U.convertTo(mat8U, CV_8UC3);  // 转换为 8 位 3 通道
    cv::imwrite(filename, mat8U);
}
...
void ProcessProblem(const std::string &dense_folder, const Problem &problem, bool geom_consistency, int patch_size)
{
    ...
    if (geom_consistency) { // False, iterate 6 times
        pro.SetGeomConsistencyParams();
    }
    pro.InuputInitialization(dense_folder, problem); // 读ref depth png， （1 ref + 多src）images jpg以及对应camera RtK

    pro.CudaSpaceInitialization();

    pro.RunPatchMatch();


    // pro.RefinePatchMatch();
    ...
    // cv::imshow("image", normals);
    // cv::waitKey(0);    

    saveFloatMatAsPNG(depths, result_folder + "/depths.png");
    saveFloat3ChannelMatAsPNG(normals, result_folder + "/normals.png");
    saveFloatMatAsPNG(costs, result_folder + "/costs.png");
...
int main(int argc, char** argv) // dense_folder, ref_id, src_ids, patch_size
{
    if (argc < 2) {
