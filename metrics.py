...
def evaluate(split='test'):
    ...
    for method in os.listdir(test_dir):
        ...
        for cam_info in tqdm(cam_infos, desc="Reading image progress"):
            ...
            # obj_bound_path = os.path.join(dataset.source_path, "dynamic_mask_15", f'{image_name}.png')
            obj_bound_path = os.path.join(dataset.source_path, "dynamic_masks/all", f'{image_name}.png')
            
            render = Image.open(render_path)
            gt = Image.open(gt_path)
            obj_bound = Image.open(obj_bound_path).resize(gt.size)
            
            renders.append(tf.to_tensor(render)[:3, :, :])
            gts.append(tf.to_tensor(gt)[:3, :, :])
            obj_bounds.append(tf.to_tensor(obj_bound)[:1, :, :].bool())
        ...
        for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
            ...
            gt = gts[idx].cuda()
            obj_mask = obj_bounds[idx].cuda()

            ssims.append(ssim(render, gt))
            psnrs.append(psnr(render, gt))
            if obj_mask.sum() != 0:
                psnrs_obj.append(psnr(render, gt, obj_mask))
            lpipss.append(lpips(render, gt, net_type='alex'))
        
        print("  SSIM : {:>12.7f}".format(torch.tensor(ssims).mean(), ".5"))
        print("  PSNR : {:>12.7f}".format(torch.tensor(psnrs).mean(), ".5"))
        print("  PSNR(OBJ) : {:>12.7f}".format(torch.tensor(psnrs_obj).mean(), ".5"))
        print("  LPIPS: {:>12.7f}".format(torch.tensor(lpipss).mean(), ".5"))
        print("")
        
        full_dict[scene_dir][method].update({"SSIM": torch.tensor(ssims).mean().item(),
                                        "PSNR": torch.tensor(psnrs).mean().item(),
                                        "PSNR(OBJ)": torch.tensor(psnrs_obj).mean().item(),
                                        "LPIPS": torch.tensor(lpipss).mean().item()})
        per_view_dict[scene_dir][method].update({"SSIM": {name: ssim for ssim, name in zip(torch.tensor(ssims).tolist(), image_names)},
                                                    "PSNR": {name: psnr for psnr, name in zip(torch.tensor(psnrs).tolist(), image_names)},
                                                    "PSNR(OBJ)": {name: psnr for psnr, name in zip(torch.tensor(psnrs_obj).tolist(), image_names)},
